...
import { type Formats, getColorString } from './utils';

// 暗色色板

// 动态梯度算法
export default function colorPaletteDark(originColor: string, i: number, format: Formats) {
  ...
  const baseColor: ColorInstance = Color.hsv(originBaseColor.hue(), getNewSaturation(6), originBaseColor.value());
  ...
  const retColor = Color.hsv(lightColor.hue(), getNewSaturation(i), lightColor.value());
