...
// 动态梯度算法
export default function colorPalette(originColor: string, i: number, format?: Formats) {
  ...
  const retColor =
    i === 6
      ? color
      : Color.hsv(getNewHue(isLight, index), getNewSaturation(isLight, index), getNewValue(isLight, index));
